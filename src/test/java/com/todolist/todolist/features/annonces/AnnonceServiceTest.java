...
class AnnonceServiceTest {
    ...
    void setUp() {
        ...
        mockCategory.setLabel("Immobilier");

        mockAnnonce = Annonce.builder()
                .id(100L)
    ...
    void searchAnnonces_noFilter_returnsPaginatedResponse() {
        ...
        when(annonceRepository.searchWithFilters(eq(entityManager), isNull(), isNull(), isNull(), eq(0), eq(5)))
                .thenReturn(List.of(mockAnnonce));
        ...
        try (MockedStatic<AnnonceMapper> mapperMock = mockStatic(AnnonceMapper.class)) {
            mapperMock.when(() -> AnnonceMapper.toDTO(mockAnnonce)).thenReturn(mockAnnonceDTO);
    ...
    void findOne_existingId_returnsDTO() {
        when(entityManagerFactory.createEntityManager()).thenReturn(entityManager);
        when(annonceRepository.findOne(entityManager, 100L)).thenReturn(mockAnnonce);
    ...
    void createAnnonce_validDTO_returnsCreatedDTO() {
        ...
        when(categoryRepository.findOne(10L)).thenReturn(mockCategory);
        when(annonceRepository.create(eq(entityManager), any(Annonce.class))).thenReturn(mockAnnonce);

        try (MockedStatic<AnnonceMapper> mapperMock = mockStatic(AnnonceMapper.class)) {
            mapperMock.when(() -> AnnonceMapper.toEntity(mockAnnonceDTO)).thenReturn(mockAnnonce);
            mapperMock.when(() -> AnnonceMapper.toDTO(mockAnnonce)).thenReturn(mockAnnonceDTO);
    ...
    void updateAnnonce_draftByOwner_updatesFieldsAndStatus() {
        ...
        when(userRepository.findOne(1L)).thenReturn(mockUser);
        when(categoryRepository.findOne(10L)).thenReturn(mockCategory);
        when(annonceRepository.update(eq(entityManager), any(Annonce.class))).thenReturn(mockAnnonce);
        ...
        try (MockedStatic<AnnonceMapper> mapperMock = mockStatic(AnnonceMapper.class)) {
            ...
            assertNotNull(result);
            assertEquals("Nouveau titre", mockAnnonce.getTitle());
            assertEquals("Nouvelle description", mockAnnonce.getDescription());
            assertEquals(StatusEnum.PUBLISHED, mockAnnonce.getStatus());
            verify(entityTransaction).commit();
        }
    }
    ...
    void updateAnnonce_publishedByOwner_onlyUpdatesStatus() {
        mockAnnonce.setStatus(StatusEnum.PUBLISHED);
        ...
        try (MockedStatic<AnnonceMapper> mapperMock = mockStatic(AnnonceMapper.class)) {
            ...
            assertEquals("Appartement centre-ville", mockAnnonce.getTitle());
            assertEquals(StatusEnum.ARCHIVED, mockAnnonce.getStatus());
            verify(entityTransaction).commit();
        }
    }
    ...
    void deleteAnnonce_archivedByOwner_deletesSuccessfully() {
        mockAnnonce.setStatus(StatusEnum.ARCHIVED);
